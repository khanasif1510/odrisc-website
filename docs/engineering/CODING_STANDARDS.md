...
```ts
import { Button, ProviderHero, submitDemoRequest, ProductStatus } from "@/index";
```
...
```ts
function createCanonicalUrl({ locale, routeKey, siteUrl }: CreateCanonicalUrlOptions): URL {
  // ...
}
```
